...
@Component({
  selector: 'app-auth-sub',
  templateUrl: './auth-sub.component.html',
  styleUrls: ['./auth-sub.component.scss']
})
export class AuthSubComponent implements OnInit {
  ...
  onSubmit(id) {
      this.authService.addUser(this.authSubscribeForm.value).subscribe(res => {
        if (res['success']){
          this.router.navigate(['/']);
        }
        else {
          this.formatted_message = res['message'];
        }
      })
  }
  onChanges() {
